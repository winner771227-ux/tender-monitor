...
_SEARCH_URL = "https://www.profilzadavatele-vz.cz/vyhledavani/?co={keyword}&datum_od={date_from}"
...
class ProfilZadavateleVzScraper(BaseScraper):
    ...
    async def scrape_page(self, page: Page) -> list[Tender]:
        ...
        for keyword in self.keywords:
            ...
            logger.info("ProfilVZ hledám: '%s' od %s", keyword, date_from)
            try:
                await page.goto(url, wait_until="domcontentloaded", timeout=self.timeout_ms)
                batch = await self._scrape_keyword(page, keyword)
                logger.info("ProfilVZ keyword='%s' nalezeno=%s", keyword, len(batch))
                all_tenders.extend(batch)
            except Exception as exc:
                logger.warning("ProfilVZ keyword='%s' chyba: %s", keyword, exc)
