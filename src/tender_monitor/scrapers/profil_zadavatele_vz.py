"""Scraper for Profil zadavatele VZ."""
    ...
    async def scrape_page(self, page: Page) -> list[Tender]:
        ...
        for _ in range(self.max_pages):
            ...
            card_batch = await self.collect_card_tenders(
                page, ".tender, .zakazka, .verejna-zakazka, .contract, article, .card, .list-item", detail_selector=detail_selector
            )
            tenders.extend(card_batch)
            logger.info("Profil zadavatele VZ page=%s batch=%s total=%s", page.url, len(batch) + len(card_batch), len(tenders))
