...
class TenderMarketScraper(BaseScraper):
    ...
    url = "https://tendermarket.cz"

    async def scrape_page(self, page: Page) -> list[Tender]:
        tenders: list[Tender] = []
        visited_urls: set[str] = set()
        table_xpath = "//table[.//th[contains(normalize-space(.), 'Zakázka') or contains(normalize-space(.), 'Zadavatel')]]"
        ...
        for _ in range(self.max_pages):
            ...
            card_batch = await self.collect_card_tenders(
                page, ".tender, .zakazka, .contract, article, .card, .list-group-item, .list-item", detail_selector=detail_selector
            )
            tenders.extend(card_batch)
            logger.info("Tender Market page=%s batch=%s total=%s", page.url, len(batch) + len(card_batch), len(tenders))
